...
export default function NavBar(props) {
    ...
    return (
      <nav data-aos='fade-in' data-aos-delay='300' className={navbar ? 'mt-2 rounded-3xl bg-white xl:py-2 z-20 px-7 pt-4 lg:pt-4 lg:py-3 md:px-20 lg:px-40 dark:bg-[#070a18] fixed w-full left-0 bg-opacity-70 dark:bg-opacity-50 dark:backdrop-blur-sm backdrop-blur-md xl:shadow-light-button dark:shadow-none transition-colors duration-1000' : 'mt-1 py-2 z-20 px-7 pt-4 lg:py-5 md:px-20 lg:px-40 transition-colors duration-1000 fixed w-full left-0'}>
      <div className="relative container flex flex-wrap items-center justify-between mx-auto pb-2 ">
        <button onClick={() => {window.scrollTo({top: 0, behavior: 'smooth', duration: 100})}}>
          <h1 className='text-2xl mb-1 xl:mb-0 lg:text-2xl font-poppins font-bold text-theme cursor-pointer bg-clip-text dark:text-transparent dark:bg-gradient-to-r dark:from-[#3a7bd5] dark:to-[#2997c5]'>Eric Xie.</h1>
        </button>
      ...
      >
        <div className="relative">
          ...
          <div ref={menuRef} onClick={() => setIsOpen(!isOpen)} className={isOpen ? 'relative xl:hidden ring-1 ring-zinc-900/5  font-poppins font-normal bg-white mt-3 py-2 text-theme dark:text-gray-200 shadow-lg shadow-zinc-800/5 rounded-2xl dark:bg-[#060813] text-center' : 'hidden'}>
          <ul>
            ...
            <li><ScrollLink onClick={() => setIsOpen(false)} to="contact" spy={true} smooth={true} offset={0} duration={700} className='block py-2 px-4 text-sm font-poppins cursor-pointer hover:text-dark dark:hover:text-gray-400'>Contact</ScrollLink></li>
          </ul>
        </div>
      </div>
